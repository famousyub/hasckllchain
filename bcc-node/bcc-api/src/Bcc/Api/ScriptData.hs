...
data ScriptData = ScriptDataConstructor Integer [ScriptData]
  ...
  -- Note the order of constructors is the same as the Zerepoch definitions
  -- so that the Ord instance is consistent with the Zerepoch one.
...
toAurumData = Aurum.Data . toZerepochData

fromAurumData :: Aurum.Data ledgerera -> ScriptData
fromAurumData = fromZerepochData . Aurum.getZerepochData


toZerepochData :: ScriptData -> Zerepoch.Data
toZerepochData (ScriptDataConstructor int xs)
                                  = Zerepoch.Constr int
                                      [ toZerepochData x | x <- xs ]
toZerepochData (ScriptDataMap  kvs) = Zerepoch.Map
                                      [ (toZerepochData k, toZerepochData v)
                                      | (k,v) <- kvs ]
toZerepochData (ScriptDataList  xs) = Zerepoch.List
                                      [ toZerepochData x | x <- xs ]
toZerepochData (ScriptDataNumber n) = Zerepoch.I n
toZerepochData (ScriptDataBytes bs) = Zerepoch.B bs


fromZerepochData :: Zerepoch.Data -> ScriptData
fromZerepochData (Zerepoch.Constr int xs)
                                = ScriptDataConstructor int
                                    [ fromZerepochData x | x <- xs ]
fromZerepochData (Zerepoch.Map kvs) = ScriptDataMap
                                    [ (fromZerepochData k, fromZerepochData v)
                                    | (k,v) <- kvs ]
fromZerepochData (Zerepoch.List xs) = ScriptDataList
                                    [ fromZerepochData x | x <- xs ]
fromZerepochData (Zerepoch.I     n) = ScriptDataNumber n
fromZerepochData (Zerepoch.B    bs) = ScriptDataBytes bs
...
-- representing constructors for Zerepoch data values.
--
