...
--  connectToRemoteNode,
    ...
    -- *** Helpers
    --TODO: These should be exported via Bcc.Api.Mode
    ConsensusMode(..),
    consensusModeOnly,
...
import qualified Shardagnostic.Network.Mux as Net
import           Shardagnostic.Network.NodeToClient (NodeToClientProtocols (..),
                   NodeToClientVersionData (..))
import qualified Shardagnostic.Network.NodeToClient as Net
import           Shardagnostic.Network.NodeToClient.Version (NodeToClientVersion (..))
import           Shardagnostic.Network.Protocol.ChainSync.Client as Net.Sync
import           Shardagnostic.Network.Protocol.ChainSync.ClientPipelined as Net.SyncP
import           Shardagnostic.Network.Protocol.LocalStateQuery.Client (LocalStateQueryClient (..))
import qualified Shardagnostic.Network.Protocol.LocalStateQuery.Client as Net.Query
import           Shardagnostic.Network.Protocol.LocalStateQuery.Type (AcquireFailure (..))
import qualified Shardagnostic.Network.Protocol.LocalStateQuery.Type as Net.Query
import           Shardagnostic.Network.Protocol.LocalTxSubmission.Client (LocalTxSubmissionClient (..),
                   SubmitResult (..))
...
import qualified Shardagnostic.Consensus.Network.NodeToClient as Consensus
import qualified Shardagnostic.Consensus.Node.NetworkProtocolVersion as Consensus
import qualified Shardagnostic.Consensus.Node.ProtocolInfo as Consensus
import qualified Shardagnostic.Consensus.Node.Run as Consensus
...
consensusModeOnly :: ConsensusModeParams mode
                  -> ConsensusMode       mode
consensusModeOnly ColeModeParams{}   = ColeMode
consensusModeOnly SophieModeParams{} = SophieMode
consensusModeOnly BccModeParams{} = BccMode
...
mkLocalNodeClientParams modeparams clients =
    ...
    --
    case modeparams of
      ColeModeParams epochSlots ->
        LocalNodeClientParams
          (ProtocolClientInfoArgsCole epochSlots)
          (convLocalNodeClientProtocols ColeMode . clients)

      SophieModeParams ->
        LocalNodeClientParams
          ProtocolClientInfoArgsSophie
          (convLocalNodeClientProtocols SophieMode . clients)

      BccModeParams epochSlots ->
        LocalNodeClientParams
          (ProtocolClientInfoArgsBcc epochSlots)
          (convLocalNodeClientProtocols BccMode . clients)
