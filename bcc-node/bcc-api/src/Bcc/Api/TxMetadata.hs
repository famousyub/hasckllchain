...
instance SerialiseAsCBOR TxMetadata where
    serialiseToCBOR =
        ...
        -- The Sophie (Metadata era) is always polymorphic in era,
...
toSophieMetadata = Map.map toSophieMetadatum

toSophieMetadatum :: TxMetadataValue -> Sophie.Metadatum
toSophieMetadatum (TxMetaNumber x) = Sophie.I x
toSophieMetadatum (TxMetaBytes  x) = Sophie.B x
toSophieMetadatum (TxMetaText   x) = Sophie.S x
toSophieMetadatum (TxMetaList  xs) = Sophie.List
                                        [ toSophieMetadatum x | x <- xs ]
toSophieMetadatum (TxMetaMap   xs) = Sophie.Map
                                        [ (toSophieMetadatum k,
                                           toSophieMetadatum v)
                                        | (k,v) <- xs ]

fromSophieMetadata :: Map Word64 Sophie.Metadatum -> Map Word64 TxMetadataValue
fromSophieMetadata = Map.Lazy.map fromSophieMetadatum

fromSophieMetadatum :: Sophie.Metadatum -> TxMetadataValue
fromSophieMetadatum (Sophie.I     x) = TxMetaNumber x
fromSophieMetadatum (Sophie.B     x) = TxMetaBytes  x
fromSophieMetadatum (Sophie.S     x) = TxMetaText   x
fromSophieMetadatum (Sophie.List xs) = TxMetaList
                                           [ fromSophieMetadatum x | x <- xs ]
fromSophieMetadatum (Sophie.Map  xs) = TxMetaMap
                                           [ (fromSophieMetadatum k,
                                              fromSophieMetadatum v)
                                           | (k,v) <- xs ]
