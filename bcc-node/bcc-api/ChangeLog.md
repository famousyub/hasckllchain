...
- Improve the error generated when a Tx output does not meet the minimum UTxO
  value. (#3027)
- Add support for querying the Aurum ledger state. (#2974)
- Update the API documentation.

## 1.28 -- July 2021

- Support for the upcoming Aurum era, including protocol parameters, Zerepoch
...
- The bcc-submit-api now takes transactions encoded as CBOR rather than
...
## 1.26.0 -- March 2020
- Added a demo for the use of bcc-client. This is an API to allow writing
  programs to interact with the bcc node. (#2295, #2303)
...
- Fix the getTxId implementation for Cole-era transactions (#2169)

## 1.24.0 -- December 2020

- Full API support for the Evie and Jen eras, including creating
...
- Preliminary support for the Evie script language extensions (#2069)
- Preliminary support for the Jen multi-asset extensions (#2083, #2085, #2093)
...
- Adjust what is exposed via Bcc.Api.{Cole,Sophie} (#1932)
...
- Support for Cole witnesses for addresses that use attributes, which includes
  all addresses in legacy Klarity Cole wallets (#1851, #1871)
- Introduce a Bcc.Api top level module exporting only the public parts
  and modules Bcc.Api.{Cole,Sophie} that expose the underlying library
...
- Support for Cole witnesses in Sophie txs in the typed API (#1339)
...
- Moving code around to eliminate the bcc-config package (#1289, #1380)
...
No changes in the bcc-api. There were changes in the bcc-node.
...
- Initial transaction API with Cole support and Sophie stubs (#787)
- Sophie address key pair generation (#799)
