...
phase_prefetch()
{
        ...
        then oprint "local ChainDB preloaded with mainnet chain up to ${limit_desc}"
                 ...
                 echo -e '\n' >&2
                 pgrep -fal 'bcc-node'
                 echo -e '\nMainnet prefetch failed, perhaps some unexpected "bcc-node" processes are running (see above)?\nIf so, you may consider:  pkill bcc-node' >&2
                 exit 1
        fi
}
...
check_port_availability "${node_port}" || {
        ...
        fprint "perhaps another 'bcc-node' is running?  netstat -pltn | grep ':${node_port} '"
        exit 1
}
...
else prebuild 'bcc-node' 'bcc-node'; fi
