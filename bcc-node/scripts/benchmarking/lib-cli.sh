...
genesis() {
        ...
        run_quiet bcc-cli bcc-cli cole genesis genesis "${ARGS[@]}"

        # move new genesis to configuration
        local GENHASH=$(run_quiet bcc-cli bcc-cli print-genesis-hash --genesis-json "${tmpdir}/genesis.json" | tail -1)
        mkdir -p "${outdir}"
...
get_default_key_address() {
        ...
        run_quiet bcc-cli bcc-cli "${ARGS[@]}" | head -n1 | xargs echo -n
}

issue_genesis_utxo_expenditure() {
        dprint "issue_genesis_utxo_expenditure: $*"
        local config="$1" tx="$2"
        local from_key="$3" from_addr="$4" entropic="$5"
        local to_addr="$6"
        ...
        local ARGS=(
                ...
                --txout               "(\"${to_addr}\",${entropic})"
        )
        run_quiet bcc-cli bcc-cli "${ARGS[@]}"
}

issue_utxo_expenditure() {
        dprint "issue_utxo_expenditure: $*"
        local config="$1" tx="$2"
        local from_key="$3" txin="$4" txinoutputindex="$5" entropic="$6"
        local to_addr="$7"
