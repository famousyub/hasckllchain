...
## rob the bank
default_entropic="862000000000000"
...
case $# in
        ...
        6 ) tx="$1"
            ...
            entropic="$6";;
        * ) cat >&2 <<EOF
Usage:  $(basename "$0") TX-FILE IN-TXID IN-INDEX FROM-KEY-FILE TO-KEY-FILE ENTROPIC
EOF
            exit 1;; esac
...
issue_utxo_expenditure \
  "${CONFIG}" "${tx}" "${from_key}" "${txin}" "${outindex}" "${entropic}" "${to_addr}"
