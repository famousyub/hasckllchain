...
mkdir -p "$WORKD"
zerepochscriptinuse=scripts/zerepoch/scripts/minting-context-equivalance-test.zerepoch
...
bcc-cli transaction build \
  ...
  --required-signer "bcc-cli/test/data/golden/sophie/keys/payment_keys/signing_key" \
  --change-address "$utxoaddr" \
...
bcc-cli transaction sign \
  ...
  --signing-key-file "bcc-cli/test/data/golden/sophie/keys/payment_keys/signing_key" \
...
bcc-cli transaction build \
  ...
  --mint "5 $policyid.MillarCoin" \
  --protocol-params-file example/pparams.json \
  --out-file "$work/zerepochmint-final.body"

bcc-cli transaction sign \
  --tx-body-file "$work/zerepochmint-final.body" \
  --testnet-magic 42 \
  --signing-key-file "$targetskey" \
  --signing-key-file "bcc-cli/test/data/golden/sophie/keys/payment_keys/signing_key" \
  --out-file "$work/zerepochmint-final.tx"

# SUBMIT
bcc-cli transaction submit --tx-file "$work/zerepochmint-final.tx" --testnet-magic 42
