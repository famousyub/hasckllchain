...
# This script sets up a cluster that starts out in Cole, and can transition to Sophie.
#
...
#
# There are three ways of triggering the transition to Sophie:
...
#    Sophie at protocol version 1.0.0, by uncommenting the line containing
#    'TestSophieHardForkAtVersion' below. Then, we just need to execute step a)
...
#    To do this, uncomment the line containing 'TestSophieHardForkAtEpoch'
...
#
# TODO: The script allows transitioning to Sophie, but not yet to register a
# pool and delegate, so all blocks will still be produced by the BFT nodes.
# We will need CLI support for Cole witnesses in Sophie transactions to do
# that.
...
FUNDS_PER_COLE_ADDRESS=$((${FUNDS_PER_GENESIS_ADDRESS} * 9 / 10))
...
sed -i ${ROOT}/configuration.yaml \
    -e 's/Protocol: RealPBFT/Protocol: Bcc/' \
    -e '/Protocol/ aPBftSignatureThreshold: 0.6' \
    -e 's/minSeverity: Info/minSeverity: Debug/' \
    -e 's|GenesisFile: genesis.json|ColeGenesisFile: cole/genesis.json|' \
    -e '/ColeGenesisFile/ aSophieGenesisFile: sophie/genesis.json' \
...
for NODE in ${ALL_NODES}; do
  ...
  mkdir ${NODE} ${NODE}/cole ${NODE}/sophie
...
cat > cole.genesis.spec.json <<EOF
{
...
mv cole.genesis.spec.json cole/genesis.spec.json


# Symlink the BFT operator keys from the genesis delegates, for uniformity
for N in ${BFT_NODES_N}; do

  ln -s ../../cole/delegate-keys.00$((${N} - 1)).key     node-bft${N}/cole/delegate.key
  ln -s ../../cole/delegation-cert.00$((${N} - 1)).json  node-bft${N}/cole/delegate.cert
...
for N in ${BFT_NODES_N}; do
  ...
  bcc-cli keygen \
    --secret cole/payment-keys.00$((${N} - 1)).key \

  bcc-cli signing-key-address \
    --testnet-magic 42 \
    --secret cole/payment-keys.00$((${N} - 1)).key > cole/address-00$((${N} - 1))
...
bcc-cli issue-genesis-utxo-expenditure \
            --genesis-json cole/genesis.json \
            --testnet-magic 42 \
            --tx tx0.tx \
            --wallet-key cole/delegate-keys.000.key \
            --rich-addr-from $(head -n 1 cole/genesis-address-000) \
            --txout "(\"$(head -n 1 cole/address-000)\", $FUNDS_PER_COLE_ADDRESS)"

# Update Proposal and votes
bcc-cli cole governance create-update-proposal \
...
for N in ${BFT_NODES_N}; do
    bcc-cli cole governance create-proposal-vote \
                --proposal-filepath update-proposal \
                --testnet-magic 42 \
                --signing-key cole/delegate-keys.00$((${N} - 1)).key \
                --vote-yes \
...
# Set up our template
mkdir sophie
bcc-cli genesis create --testnet-magic 42 --genesis-dir sophie
...
if [ -f ${NODE}/cole/delegate.key ]; then
  echo "  --signing-key                     ${ROOT}/${NODE}/cole/delegate.key \\"
  echo "  --delegation-certificate          ${ROOT}/${NODE}/cole/delegate.cert \\"
fi
  echo "  --sophie-kes-key                 ${ROOT}/${NODE}/sophie/kes.skey \\"
  echo "  --sophie-vrf-key                 ${ROOT}/${NODE}/sophie/vrf.skey \\"
  echo "  --sophie-operational-certificate ${ROOT}/${NODE}/sophie/node.cert \\"
...
for NODE in ${POOL_NODES}; do
  ...
  bcc-cli node key-gen \
      --cold-verification-key-file                 ${NODE}/sophie/operator.vkey \
      --cold-signing-key-file                      ${NODE}/sophie/operator.skey \
      --operational-certificate-issue-counter-file ${NODE}/sophie/operator.counter

  bcc-cli node key-gen-VRF \
      --verification-key-file ${NODE}/sophie/vrf.vkey \
      --signing-key-file      ${NODE}/sophie/vrf.skey
...
for N in ${BFT_NODES_N}; do
  ...
  ln -s ../../sophie/delegate-keys/delegate${N}.skey node-bft${N}/sophie/operator.skey
  ln -s ../../sophie/delegate-keys/delegate${N}.vkey node-bft${N}/sophie/operator.vkey
  ln -s ../../sophie/delegate-keys/delegate${N}.counter node-bft${N}/sophie/operator.counter
  ln -s ../../sophie/delegate-keys/delegate${N}.vrf.vkey node-bft${N}/sophie/vrf.vkey
  ln -s ../../sophie/delegate-keys/delegate${N}.vrf.skey node-bft${N}/sophie/vrf.skey
...
for NODE in ${ALL_NODES}; do
  ...
  bcc-cli node key-gen-KES \
      --verification-key-file ${NODE}/sophie/kes.vkey \
      --signing-key-file      ${NODE}/sophie/kes.skey

  bcc-cli node issue-op-cert \
      --kes-period 0 \
      --kes-verification-key-file                  ${NODE}/sophie/kes.vkey \
      --cold-signing-key-file                      ${NODE}/sophie/operator.skey \
      --operational-certificate-issue-counter-file ${NODE}/sophie/operator.counter \
      --out-file                                   ${NODE}/sophie/node.cert
...
for N in ${USER_POOL_N}; do
  ...
  bcc-cli stake-address delegation-certificate \
      ...
      --cold-verification-key-file  node-pool${N}/sophie/operator.vkey \
...
for NODE in ${POOL_NODES}; do
  ...
  bcc-cli stake-pool registration-certificate \
    ...
    --cold-verification-key-file             ${NODE}/sophie/operator.vkey \
    --vrf-verification-key-file              ${NODE}/sophie/vrf.vkey \
...
bcc-cli transaction build-raw \
    ...
    --tx-in $(bcc-cli genesis initial-txin \
                --testnet-magic 42 \
                --verification-key-file sophie/utxo-keys/utxo1.vkey) \
...
# TODO: this will become the transaction to register the pool, etc. We'll need to pick the tx-in from the actual UTxO -- since it contains the txid, we'll have to query this via bcc-cli query utxo.
# bcc-cli transaction build-raw --invalid-hereafter 1000000 --fee 0 --tx-in 67209bfcdf78f8cd86f649da75053a80fb9bb3fad68465554f9301c31b496c65#0 --tx-out $(cat example/addresses/user1.addr)+450000000 --certificate-file example/addresses/pool-owner1-stake.reg.cert     --certificate-file example/node-pool1/registration.cert     --certificate-file example/addresses/user1-stake.reg.cert     --certificate-file example/addresses/user1-stake.deleg.cert --out-file example/register-pool.txbody

# bcc-cli address convert --cole-key-file example/cole/payment-keys.000.key --signing-key-file example/cole/payment-keys.000-converted.key

# bcc-cli transaction sign --tx-body-file example/register-pool.txbody --testnet-magic 42 --signing-key-file example/cole/payment-keys.000-converted.key --signing-key-file example/sophie/utxo-keys/utxo1.skey --signing-key-file example/addresses/user1-stake.skey --signing-key-file example/node-pool1/owner.skey --signing-key-file example/node-pool1/sophie/operator.skey --out-file example/register-pool.tx

# bcc-cli transaction submit --tx-file example/register-pool.tx --testnet-magic 42
...
echo
echo "BCC_NODE_SOCKET_PATH=${ROOT}/node-bft1/node.sock \\"
echo "  bcc-cli cole submit-update-proposal --testnet-magic 42 --filepath ${ROOT}/update-proposal"
echo
echo "To submit votes on the update proposal"
echo
for N in ${BFT_NODES_N}; do
    echo "BCC_NODE_SOCKET_PATH=${ROOT}/node-bft1/node.sock \\"
    echo "bcc-cli cole submit-proposal-vote  --testnet-magic 42 --filepath ${ROOT}/update-vote.00$((${N} - 1))"
done
echo
...
echo
echo "BCC_NODE_SOCKET_PATH=${ROOT}/node-bft1/node.sock \\"
echo "  bcc-cli cole submit-update-proposal --testnet-magic 42 --filepath ${ROOT}/update-proposal-1"
echo
echo "To submit votes on the update proposal"
echo
for N in ${BFT_NODES_N}; do
    echo "BCC_NODE_SOCKET_PATH=${ROOT}/node-bft1/node.sock \\"
    echo "bcc-cli cole submit-proposal-vote  --testnet-magic 42 --filepath ${ROOT}/update-vote-1.00$((${N} - 1))"
done
echo
echo "To submit the transaction that registers the pool, in the Sophie era"
echo
echo "BCC_NODE_SOCKET_PATH=${ROOT}/node-bft1/node.sock \\"
