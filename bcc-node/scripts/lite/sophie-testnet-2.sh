...
bcc_cli="$(cabal exec -- which bcc-cli)"
bcc_node="$(cabal exec -- which bcc-node)"
...
# Set up our template
$bcc_cli genesis create --testnet-magic 42 --genesis-dir ${ROOT}
...
for ADDR in ${ADDRS}; do
  ...
  # Stake addresses
  $bcc_cli stake-address build \
      --stake-verification-key-file addresses/${ADDR}-stake.vkey \
      --testnet-magic 42 \
      --out-file addresses/${ADDR}-stake.addr

  # Stake addresses registration certs
  $bcc_cli stake-address registration-certificate \
      --stake-verification-key-file addresses/${ADDR}-stake.vkey \
...
for N in ${USER_POOL_N}; do
  ...
  $bcc_cli stake-address delegation-certificate \
...
for NODE in ${POOL_NODES}; do
  $bcc_cli stake-pool registration-certificate \
    --testnet-magic 42 \
...
$bcc_cli transaction build-raw \
    --invalid-hereafter 1000 \
    --fee 0 \
    --tx-in $($bcc_cli genesis initial-txin \
                --testnet-magic 42 \
...
for NODE in ${ALL_NODES}; do
  ...
  $bcc_node run \
    ...
    --sophie-kes-key                 ${ROOT}/${NODE}/kes.skey \
    --sophie-vrf-key                 ${ROOT}/${NODE}/vrf.skey \
    --sophie-operational-certificate ${ROOT}/${NODE}/node.cert \
