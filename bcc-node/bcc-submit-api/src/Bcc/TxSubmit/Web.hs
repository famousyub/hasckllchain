...
import           Bcc.TxSubmit.CLI.Types (SocketPath (SocketPath))
import           Bcc.TxSubmit.Metrics (TxSubmitMetrics (..))
import           Bcc.TxSubmit.Rest.Types (WebserverConfig (..), toWarpSettings)
import           Bcc.TxSubmit.Types (EnvSocketError (..), RawCborDecodeError (..),
                   ...
                   renderTxCmdError)
import           Bcc.TxSubmit.Util (logException)
...
import qualified Bcc.TxSubmit.Rest.Web as Web
...
readByteStringTx :: ByteString -> ExceptT TxCmdError IO (InAnyBccEra Tx)
readByteStringTx = firstExceptT TxCmdTxReadError . hoistEither . deserialiseAnyOf
  [ FromSomeType (AsTx AsColeEra)   (InAnyBccEra ColeEra)
  , FromSomeType (AsTx AsSophieEra) (InAnyBccEra SophieEra)
  , FromSomeType (AsTx AsEvieEra) (InAnyBccEra EvieEra)
  , FromSomeType (AsTx AsJenEra)    (InAnyBccEra JenEra)
...
txSubmitPost trace metrics (AnyConsensusModeParams cModeParams) networkId (SocketPath socketPath) txBytes = handle $ do
    InAnyBccEra era tx <- readByteStringTx txBytes
    let cMode = AnyConsensusMode $ consensusModeOnly cModeParams
    eraInMode <- hoistMaybe
                   (TxCmdEraConsensusModeMismatch cMode (AnyBccEra era))
