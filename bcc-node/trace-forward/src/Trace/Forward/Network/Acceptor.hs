...
import           Shardagnostic.Network.ErrorPolicy (nullErrorPolicies)
...
import           Shardagnostic.Network.Socket (AcceptedConnectionsLimit (..),
...
import           Shardagnostic.Network.Protocol.Handshake.Version (acceptableVersion,
...
listenToForwarder iomgr config@AcceptorConfiguration{forwarderEndpoint} loHandler niHandler = do
 ...
 where
  app =
    ...
    -- 'AcceptorConfiguration'.  Currently 'shardagnostic-network' does not exposes
...
doListenToForwarder
  ...
  -> ShardagnosticApplication 'ResponderMode addr LBS.ByteString IO Void ()
  -> IO ()
