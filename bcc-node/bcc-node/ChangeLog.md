...
- Update the genesis hash mismatch error for Aurum. (#3003)
...
- Fix the version of Zerepoch to be used in release. (#2369, #2372, #2384, #2420,
  #2425)
- Rename the `IsValidating` field in Aurum to `IsValid`. (#2396)
- Canonically encode the subset of protocol parameters presented to Zerepoch
...
- Various internal testing improvements. (#2377, #2398, #2399, #2402, #2403,
  #2404)
- Testing for the Aurum era. (#2348, #2380, #2390)
- Various internal improvements. (#2374, #2378, #2387, #2394, #2395, #2412,
  #2413, #2421, #2422, #2423, #2428)
- Updates to the Aurum formal specification. (#2353)
...
- Update the Aurum Tx to include the validity flag, which will now be passed by
...
- Add reporting of why Zerepoch scripts fail, sufficient to locally reproduce the
  failure. (#2386, #2430)
- Fix a bug in deserialisation of witnesses. This would allow for somebody to
  ...
  have to continue to allow this in pre-Aurum eras since it has been observed
...
- Initial support for running an Aurum node. (#2642, #2649, #2657, #2738,
  #2759)
- Add a separate genesis file for Aurum. This must be present when forking to
  the Aurum era. It provides initial configuration for new parameters required
  in Aurum, such as cost models and pricing for Zerepoch scripts. (#2743, #2765)
...
- Add `bcc-tx-generator`. This is a testing utility. (#2603)
...
- Introduce the Aurum era. (#3131, #3138)
- Additional testing in the Aurum era. (#3191, #3210)
...
- Update the mempool design for compatibility with Aurum, which requires some
...
- Work on implementing the Aurum era. (#2260, #2264, #2265, #2270, #2273,
  #2274, #2277, #2283, #2287, #2294, #2295, #2296, #2299, #2302, #2303, #2304,
  #2305, #2306, #2317, #2320, #2322, #2324, #2328, #2329, #2335, #2342, #2345,
  #2355, #2359)
- Define the CDDL for Aurum datatypes. (#2281, #2315, #2337, #2362)
- Testing work for the Aurum era. (#2259, #2261, #2263, #2272, #2275, #2279,
...
- Add the ability to query the UTxO by TxIn, which is now used by the CLI.
  (#2331)
- Updated the Sophie spec with an image depicting the reward process and the
  various phases it goes through. (#2282)
- Add the ability to convert from slot time to UTC time within the forecast
  window. This will be necessary for Zerepoch scripts. (#2297)
- Add an errata to the Sophie spec, which addresses:
  - Reward calculation & stake addresses registration timing.
  - Add a note about Cole redeem addresses being returned to the reserves.
  ...
  (#2323)
- Align the Sophie spec with code. (#2339)
...
- Continued updates to the Aurum formal specification. (#2258, #2271, #2341)
...
- Preparatory work for the implementation of Shardagnostic Genesis. (#3002)
...
- Provide the ledger with the ability to perform slot to time conversion. This
  will be needed for Zerepoch scripts in the upcoming Aurum release. (#3036, #3098)
### ledger changes

- Work on implementing the upcoming Aurum era. (#2192, #2198, #2200, #2204,
  #2210, #2211, #2212, #2214, #2215, #2216, #2218, #2220, #2221, #2222, #2225,
  #2226, #2227, #2229, #2231, #2235, #2237, #2242, #2243, #2251, #2256, #2260,
  #2264)
- Released the CDDL spec for Aurum. (#2228)
- Add an optional network ID field to the transaction body. This will only come
  into use in the Aurum era. (#2219, #2254)
- Define a bcc-ledger-core package containing era-independent primitives.
  (#2207)
- Fixes to the SophieMA formal specification. (#2181)
...
- Additional properties added to the Jen/Evie formal specification (#2178)
- Updates to the Aurum formal specification (#2189, #2194)
- Work on implementing the upcoming Aurum era. (#2176, #2185, #2190)
...
- Prototype reporting metrics to EKG directly, rather than via the switchboard.
  (#2355)
- Multiple improvements to the configuration when running bcc-node as a
...
- Add an automated test for forking from Evie to Jen eras (#2915)
...
- Fixed a performance regression in the Evie and Jen eras. This resulted in
...
- Updates to the Aurum formal specification (#2108)
- A lot of work on implementing the upcoming Aurum era (#2124, #2127, #2148,
...
- Some additional support for `bcc-client`, an API to write tools for
...
### node changes
- All metrics now use a common name prefix `bcc.node.metrics`. This requires
...
- Support in testnets for skipping initial eras. This makes it easier and
  quicker to automate the setup of Bcc-mode testnets that start in later
  eras, such as Evie or Jen. This enables simplifications in the setup
...
- Initial steps of an overhaul of the automated Shardagnostic tests (#2835, #2837)
- A first draft of a technical report on the design and the internals of the
  Bcc consensus and storage layer (#2663, #2838, #2841, #2842, #2853)
- Document the existing coding style conventions (#2829, #2834, #2836)
- Fix the QueryAnytimeJen case for queries about the Jen era (#2891)
...
- Minor change to the ledger CDDL binary specification for the Jen era:
...
- Progress on the new Aurum era (#2022, #2055, #2061, #2062, #2067, #2077,
  #2087, #2088, #2095)
...
- Eliminate the need to update the `LastKnownBlockVersion-*` entries in the node
  config files for the Sophie-based eras. This means the configuration does not
  need to be updated for the Evie or Jen eras. (#2193)
...
### node changes
- Move all metrics under the `bcc.node.metrics` namespace (#2158)
...
### ledger changes
- Fix the use of Sophie multi-sig scripts in the Evie and later eras (#2035)
- Fix the serialisation format for multi-asset values to follow the CDDL (#2039)
- Additional "golden" tests for encodings in the Evie and Jen eras (#2031)
- Additional binary encoding "round-trip" tests (#2032)
- Benchmarks for the transaction generators (#2024)

### consensus changes
- Fix support for Sophie transactions in the Evie era (#2788)
...
### network changes
- Update the `bcc-ping` tool to support the node-to-node protocol V4 (#2787)
...
- Allow starting a node with the credentials for many stake pools. This is used
  ...
  not available in the "Bcc" protocol mode used for the mainnet. (#2068)

### ledger changes
- Support for optional additional scripts in the tx auxiliary data (#1993)
- Ensure the minting field in is not used in the Evie era (#2028)
- Update the CDDL specification of the blockchain binary format for Evie
  and Jen eras (#1994, #1999, #2009)
...
- Extend more Sophie tests to cover the Evie and Jen eras too (#1997,
  #2012, #2029)
- Internal refactoring and clean-ups (#2013, #2014, #2018, #2021, #2025)
- Initial preparatory steps for the Aurum era (#2016, #2027)
- Minor corrections to the formal spec arising from internal review (#2023)

### consensus changes
- Use a single set of credentials for all Sophie-based eras. This keeps the
  node configuration simple for the new eras. (#2753)
- Add the Evie era in the "ThreadNet" consensus tests (#2633, #2641)
- Add support for a Jen-only protocol mode to simplify benchmarking (#2754)
...
- Preliminary support for the upcoming Evie and Jen eras (#2038, #2080)
...
- Further development for the Evie era (#1951, #1965, #1984, #1989)
- Further development for the Jen multi-asset era (#1961, #1959, #1971, #1977,
  #1981, #1987, #1988, #1990)
...
- Support for the new Evie and Jen ledger eras (#2668, #2670, #2677, #2679)
...
### network changes
- Internal API improvements that will subsequently allow the bcc-api
...
- Preliminary support for the upcoming Evie and Jen eras (#1958)
- Documentation updates (#1613, #1968, #1960, #2015)

### consensus changes
- Preliminary support for the upcoming Evie and Jen eras (#2666)
...
- Keep a compact form of the Sophie genesis content, to allow it to be
  queried later (#2704)
...
- Preliminary support for the upcoming Evie and Jen eras (#1899)
- Preliminary support for the time-lock feature in the script language to be
  included in the Evie era (#1895)
...
- Allow most Sophie ledger rules to be reused in subsequent eras (#1922)
...
- New bcc-ping demo and tool (#2701)
...
- Fix a unlikely-but-possible bug in crossing the hard fork from Cole to
  Sophie (#2455, #2626)
- Initial support for hard forks after Sophie (#2452, #2471)
...
- Sanity check the Sophie genesis on node start up to help with configuring
...
- Refactoring to reduce tech debt (#225, #548, #2264, #2513, #2514, #2534,
  #2575, #2604)
- Improvements to the automated Sophie tests (#2462, #2557, #2567)
- Improvements to the automated Optimum tests (#2577, #2578)
...
- Improve automatic Sophie consensus tests based on ledger tests (#2490, #2503)
...
### node changes
- Properly display tx hash in Sophie UTxO query command output (#1535)
...
### consensus changes
- Account for encoding overhead of Sophie transactions (#2466)
- Optimise translating Cole to Sophie UTxOs (#2464)
...
### node changes
- Fix KES metric reporting in the Bcc mode (#1448, #1505)
- Fix rendering of block hashes in logging output (#1488)
- Trace hard fork transition events (#1520)
- Script for setting up a local cluster in Bcc mode (#1487)
...
- The initial Optimum epoch nonce is now set to the Sophie genesis hash (#1470)
...
- Fix query compatibility between Bcc and Cole modes (#2361, #2385)
...
- Improve performance of syncing in Bcc mode (#2375, #2390)
...
- Add validation for Sophie genesis configurations to avoid mistakes (#2423)
- Test improvements (#1533, #2366, #2130, #2362, #2377, #2361, #2385)
- Extend automated tests to cover Sophie d=0 environments (#2378)
...
- Finalise the format and specification of Cole address witness (#1657, #1670)
- Unclaimed epoch pool rewards go to the treasury not the reserves (#1642)
- Limit the sizes of attributes in Cole addresses (#1662)
- Simplify the calculation of the VRF seed (#1659)
- Fix the selection of the epoch nonce (#1651)
- Eliminate protocolMagicId from the Sophie genesis file (#1668)
...
No changes in the node. There were changes in the bcc-api and bcc-cli.
...
- Changes resulting from refactoring in the Bcc API library (#1289, #1316,
  #1380, #1384)
...
- Allow setting the intial Optimum nonce (#2005, #2289)
...
- Tests and other clean-up for witnesses for spending from Cole address in
  Sophie transactions (#1573, #1591)
...
### node changes
- Initial integration of the "Cole;Sophie" composite protocol (#1242, #1294)
...
### consensus changes
- Refactoring of serialisation and other details for the Cole-to-Sophie
...
### network changes
- Timeouts for chain sync are drawn from a Optimum-specific distribution (#2249)
...
- Preparation for the Cole-to-Sophie hard-fork (#2163, #2145, #2182, #2185,
  #2187, #2189)
- Basic tests for the hard fork combinator (#2063, #2079, #2134, #2167, #2197)
- Deterministic Optimum chain selection using the leader VRF (#2195)
- Fix Optimum stability window calculations (#2201, #2202, #2219, #2220)
...
- Move serialisation property tests from shardagnostic-consensus (#1497, #1506)
...
- Refactoring of ShardagnosticApplication type (#2121)
...
- Support initial Sophie protocol parameters in the Sophie genesis (#906)
...
- Memory leak detection for Sophie transactions (for mempool) (#2010)
- Tests for the Sophie consensus protocol with d < 1 (#1991, #2105)
- Split Sophie tests into a separate test library (#2129)
- Fix the construction of the Sophie initial UTxO (#2038, #2046)
- Include the Sophie protocol parameters into the Sophie genesis (#2040)
...
- Sophie-compliant handling of OCert issue number in chain selection (#2108)
...
### ledger changes (Cole)
- Repository merged with Sophie ledger repository
...
### node changes
- Sophie preparatory refactoring (#775, #792, #807)
- Sophie genesis file support (#798)
- Initial support for starting a node in Sophie mode (#820)
- Initial support for loading Sophie node leader credentials (#832)
- Split out CLI functionality into bcc-cli package (#819)
- Support for reading/writing readable Sophie key file formats (#826, #829)
...
- Sophie ledger integration improvements (#1963, #1821, #1967, #1984, #1986)
- Verify no excessive memory use in Sophie ledger state (#1558, #1928)
...
- Document Cole-era protections against network address confusion (#727, #755)
...
### node changes
- CLI support for submitting Cole era update proposals (#714)
...
- CLI support for creating Cole era update proposals (#696)
...
### consensus changes
- Sophie ledger (#258, #982, #1403, #1405, #1820, #1822, #1824, #1695)
- More internal framework to support hard forks (#1698)
- Updated Sophie support for the local state query (#1848)
- Fix subtle block fetch concurrency bug found by QC tests (#1845, #1850)
- Tests for Cole era software update proposals (#1733, #1790)
- Tests for transactions with TTLs as used by Sophie (#1297, #1564)
...
- Internal changes to prepare for the Sophie hard fork (#1750, #1775)
...
### ledger changes
- Fix calculation of transaction size to match legacy Cole implementation (#703)
...
### consensus changes
- Split consensus library: generic, cole, mock, testing  (#1651, #1652)
...
- Introduce shardagnostic-network-framework library and rearrange modules (#1655)
...
- Remove redundant `--genesis-hash` flag from `bcc-cli` (#540)
...
- Clean up EntropicPortion representation and API
...
- Change bcc-cli transaction format to be the raw chain format (#529)
...
- Correct the size calculation of Cole txs in the mempool (#1535, #1540)
...
- Reorganise shardagnostic-network modules (#1519)
...
- Fix non-liveview mode in sophie-testnet.sh script (#459)
- Elide repeated log messages (#445)
- Simplify bcc-cli interface (#476)
- Remove unneeded bcc-cli dump-hardcoded-genesis subcommand (#451)
- Remove dependency on bcc-ledger-test (#451)
...
### ledger changes
- Update to latest version of bcc-ledger-specs
...
- CI integration test for mixed cluster of old bcc-sl nodes and new nodes
...
- Finish feature to support accepting blocks from the near "future", once the
  ...
  still respecting the Shardagnostic rule of "blocks from the future" being invalid.
...
- Complete rewrite compared to previous bcc-sl series.

- New modular design. The bcc-node is the top level for the node and
