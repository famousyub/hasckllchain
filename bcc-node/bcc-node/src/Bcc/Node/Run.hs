...
setupTrace loggingLayer = do
    hn <- maybe hostname (pure . pack) =<< lookupEnv "BCC_NODE_LOGGING_HOSTNAME"
    return $
        setHostname hn $
...
handleSimpleNode scp runP trace nodeTracers nc onKernel = do
  ...
  nt <- either (\err -> panic $ "Bcc.Node.Run.handleSimpleNode.readTopologyFile: " <> err) pure eitherTopology
