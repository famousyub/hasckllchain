...
instance AdjustFilePaths NodeProtocolConfiguration where
  ...
  adjustFilePaths f (NodeProtocolConfigurationCole pc) =
    NodeProtocolConfigurationCole (adjustFilePaths f pc)

  adjustFilePaths f (NodeProtocolConfigurationSophie pc) =
    NodeProtocolConfigurationSophie (adjustFilePaths f pc)

  adjustFilePaths f (NodeProtocolConfigurationBcc pcb pcs pca pch) =
    NodeProtocolConfigurationBcc (adjustFilePaths f pcb)
                                     (adjustFilePaths f pcs)
                                     (adjustFilePaths f pca)
                                     pch

instance AdjustFilePaths NodeColeProtocolConfiguration where
  adjustFilePaths f x@NodeColeProtocolConfiguration {
                        npcColeGenesisFile
                      } =
    x { npcColeGenesisFile = adjustFilePaths f npcColeGenesisFile }

instance AdjustFilePaths NodeSophieProtocolConfiguration where
  adjustFilePaths f x@NodeSophieProtocolConfiguration {
                        npcSophieGenesisFile
                      } =
    x { npcSophieGenesisFile = adjustFilePaths f npcSophieGenesisFile }

instance AdjustFilePaths NodeAurumProtocolConfiguration where
  adjustFilePaths f x@NodeAurumProtocolConfiguration {
                        npcAurumGenesisFile
                      } =
    x { npcAurumGenesisFile = adjustFilePaths f npcAurumGenesisFile }
...
protocolName ColeProtocol   = "Cole"
protocolName SophieProtocol = "Sophie"
protocolName BccProtocol = "Cole; Sophie"
