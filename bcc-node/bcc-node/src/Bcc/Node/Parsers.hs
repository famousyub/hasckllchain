...
nodeRunParser = do
  ...
  socketFp <-   lastOption $ parseSocketPath "Path to a bcc-node socket"

  -- Protocol files
  coleCertFile   <- optional parseColeDelegationCert
  coleKeyFile    <- optional parseColeSigningKey
  sophieKESFile  <- optional parseKesKeyFilePath
  sophieVRFFile  <- optional parseVrfKeyFilePath
  sophieCertFile <- optional parseOperationalCertFilePath
  sophieBulkCredsFile <- optional parseBulkCredsFilePath
...
parseColeDelegationCert =
  strOption ( long "cole-delegation-certificate"
    <> metavar "FILEPATH"
...
parseColeSigningKey =
  strOption ( long "cole-signing-key"
            <> metavar "FILEPATH"
            <> help "Path to the Cole signing key."
            <> completer (bashCompleter "file")
            )
  <|>
...
parseOperationalCertFilePath =
  strOption
    ( long "sophie-operational-certificate"
        <> metavar "FILEPATH"
        <> help "Path to the delegation certificate."
        <> completer (bashCompleter "file")
    )
