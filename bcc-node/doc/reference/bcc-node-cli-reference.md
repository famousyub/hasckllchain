...
This section provides a reference of the core `bcc-cli` commands and their associated sub commands:

*bcc-cli*
The set of `bcc-cli` commands include:
...
* `query`: node query commands. This queries the local node whose Unix domain socket is obtained from the BCC_NODE_SOCKET_PATH environment variable.
...
* `build-raw`: builds a low-level transaction (uses the `--bcc-mode`, `--cole-mode`, `--sophie-mode` flags)
...
* `submit`: submits the transaction to the local node whose Unix domain socket is obtained from the CARANO_NODE_SOCKET_PATH environment variable (uses the `--bcc-mode`, `--cole-mode`, `--sophie-mode` flags)
