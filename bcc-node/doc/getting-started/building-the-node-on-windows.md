# Building the development version of the Bcc node on Windows

This document explains how to build a __DEVELOPMENT__ version of the `bcc-node`.
...
Go to the directory, where you cloned the `bcc-node` repository and add the command below
...
```bash
cabal build --builddir /c/dist exe:bcc-node
```
...
```bash
cabal run --builddir /c/dist exe:bcc-node -- run --help
```
