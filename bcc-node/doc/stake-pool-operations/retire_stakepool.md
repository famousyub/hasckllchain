...
```bash
> bcc-cli query tip --testnet-magic 42 --bcc-mode
{
  "epoch": 51,
...
```
curl -s -m 3 -H 'Accept: application/json' http://127.0.0.1:12788/ | jq '.bcc.node.metrics.epoch.int.val'
> 51
```
...
Create the deregistration certificate and save it as `pool-deregistration.cert`:
    ...
    bcc-cli stake-pool deregistration-certificate \
...
We query our address for a suitable UTxO to use as input:
    ...
    --mainnet
           ...
           TxHash             TxIx      Amount
    ------------------------------------------------
    9db6cf...                    0      999999267766 entropic
