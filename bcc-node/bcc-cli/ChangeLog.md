...
- The query tip command is now tidier and shows various additional pieces of
  information:
  - The epoch number is now shown during the Cole era.  Previously this worked
    only in the Sophie and subsequent eras. (#2688)
...
- Queries should now work during the Aurum era. (#2727, #2755)
- Support for submitting transactions during the Aurum era. (#2774, #2798,
  #2806, #2811, #2823, #2863, #2848)
- `bcc-cli genesis create` now also creates the new Aurum genesis file.
  (#2743)
...
- The query tip now also returns the era (e.g. Sophie, Evie, Aurum).
  (#2561, #2562, #2598)
...
- Add additional commands for creating MIR certificates to the CLI. This
  ...
  (#2503)
- As a result of refactoring in preparation for the upcoming Aurum release,
...
- Add three new queries to the CLI, exposing functionality already present in
  ...
  (#2275, #2290)
- Fix the rendering of Cole-era `TxOut`s to be consistent with the rendering for
  Sophie-era addresses. (#2472)
- Add `bcc-cli transaction view`, which allows for pretty-printing details
...
- Make the Jen era the default era in the CLI (#2415)
- Migrate the `bcc-submit-api` tool from `bcc-rest`. (#2370)
...
- Improved error messages for syntax errors in out-of-range entropic quantities
  in transaction outputs (#2063, #2079)
- Improved reference documentation for simple scripts and their use (#2165)
- Refactoring in the Cole part of the CLI to make more extensive use of the
  Bcc API and reduce the maintenance burden (#2103, #2228)
- Remove support for changing the delegation from Genesis keys to operational
  keys in the Cole era. This feature was never used on the mainnet during the
  Cole era. (#2219)
...
- New command `cole transaction txid` to help scripts with getting the
  transaction id for Cole transactions made using the cli (#2169)
...
- CLI support for the Evie and Jen eras, including creating transactions
...
- New flags for the `build-raw` command:
  + `--invalid-before` and `--invalid-hereafter` for the new Evie-era feature
    of transaction validity intervals. The existing flag `--ttl` is equivalent to
    the new `--invalid-hereafter`, but it is now optional in the Evie era.
  + `--script-file` for the new Evie-era feature of being able to include
     auxiliary scripts in a transaction.
  + `--mint` for the Jen-era token minting feature.
- It is now necessary to specify the target era (e.g. `--evie-era`) when
  creating a transaction (with `build-raw`) so that the right format and
  feature-set is used. The `--sophie-era` remains the default.
- It is necessary for now to specify the target era when using the CLI query
  commands. This may become automatic in future. The default is `--sophie-era`.
- Move all the Sophie sub-commands to the top level of the command line.
  For example `bcc-cli sophie transaction build-raw` becomes simply
  `bcc-cli transaction build-raw`. The existing names are also kept for
...
- New command `genesis create-staked` to make it easier to set up Sophie-based
...
- New command to query the Sophie protocol (not just ledger) state (#2057)
...
- Preliminary support for the upcoming Evie and Jen eras (#1958, #2019)
...
- Support for converting ITN extended keys to Sophie stake keys (#1579)
- Support for converting password-protected Cole signing keys (#1633)
...
- Reorganise the Cole CLI commands similarly to the Sophie ones (#1609, #1628)
...
- Default to the Bcc protocol for talking to a node (#1515)
...
- Support for interacting with nodes running in Cole-only, Sophie-only or
  the composite Bcc mode (Cole;Sophie) (#1435)
- Add support for cole keys and extended ed25519 keys (#1411)
...
- Fix the format of Cole addresses used in Cole CLI commands (#1326)
- Port CLI commands to use the new API (#1341, #1375, #1396, #1397)
- Change to JSON output for the "query tip" command (#1340, #1365)
- Moving code around to eliminate the bcc-config package (#1289, #1316)
...
No changes in the bcc-cli. There were changes in the bcc-node.
...
- Start to migrate to using the new typed API from bcc-api lib (#1284, #1298)
...
- Change the Sophie cli command and flag names to be more consistent (#1068)
...
- Improved human readable error messages for Sophie commands (#1021)
...
- Updated Sophie from scratch documentation (#1062)

## 1.12.0 -- May 2020

- Reorganise the `sophie` subcommands (#840, #845)
- New `sophie genesis create` command (#852, #864, #908, #926, #929)
- New key-gen commands for various Sophie  keys (#846, #870)
- New commands for Sophie  address construction (#870, #872, #887)
- New Sophie transaction sign command (#894, #900)
- New Sophie transaction submission command (#904)
...
- New command to create Sophie protocol param updates (#950, #1004)
- Cole update proposal vote creation and submission (#804)
- Various refactoring (#874, #875, #949, #958, #966, #972)
- Commands that talk to the node no longer require the node config file (#901,
  #907, #917, #913, #928)
- Improved human readable error messages for Cole commands (#1003)
- Documentation on constructing a Sophie chain from scratch (#893, #932, #1000)
...
- Split the `bcc-cli` package out of `bcc-node` (#819)
- Initial structure of Sophie CLI commands with a top-level "sophie" command
- Group Cole commands under a top-level "cole" command
- Commands to generate Sophie KES and VRF keys (#816)
- Command to generate Sophie address keys (#824)
