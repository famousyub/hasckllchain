module Bcc.CLI.Jen.ValueParser
  ( parseValue
  ) where
...
evalValueExpr vExpr =
  case vExpr of
    ...
    ValueExprEntropic quant -> valueFromList [(BccAssetId, quant)]
...
-- | Parse either a 'ValueExprEntropic' or 'ValueExprMultiAsset'.
...
valueExprTerm = do
    ...
    _ <- spaces
    pure $ case aId of
      BccAssetId -> ValueExprEntropic q
...
assetId =
    try adaAssetId
      ...
      <?> "asset ID"
  where
    -- Parse the BCC asset ID.
    adaAssetId :: Parser AssetId
    adaAssetId = string "entropic" $> BccAssetId

    -- Parse a multi-asset ID.
    nonBccAssetId :: Parser AssetId
    nonBccAssetId = do
      polId <- policyId
