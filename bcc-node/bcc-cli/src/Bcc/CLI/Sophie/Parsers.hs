...
pAddressCmd =
   asum
     ...
     , subParser "build"
         (Opt.info pAddressBuild $ Opt.progDesc "Build a Sophie payment address, with optional delegation to a stake address.")
     , subParser "build-script"
         (Opt.info pAddressBuildScript $ Opt.progDesc "Build a Sophie script address. (deprecated; use 'build' instead with '--payment-script-file')")
     , subParser "info"
...
pScriptWitnessFiles witctx autoBalanceExecUnits scriptFlagPrefix scriptFlagPrefixDeprecated help =
  ...
  where
    ...
    toScriptWitnessFiles sf (Just (d,r, e)) = ZerepochScriptWitnessFiles sf d r e
...
pKeyCmd =
    ...
    , subParser "non-extended-key" $
        Opt.info pKeyNonExtendedKey $
          Opt.progDesc $ "Get a non-extended verification key from an "
                      ...
                      ++ "extended key types."
    , subParser "convert-cole-key" $
        Opt.info pKeyConvertColeKey $
          Opt.progDesc $ "Convert a Cole payment, genesis or genesis "
                      ++ "delegate key (signing or verification) to a "
                      ++ "corresponding Sophie-format key."
    , subParser "convert-cole-genesis-vkey" $
    ...
    , subParser "convert-itn-bip32-key" $
        Opt.info pKeyConvertITNBip32Key $
          Opt.progDesc $ "Convert an Incentivized Testnet (ITN) BIP32 "
                      ...
                      ++ "Sophie stake signing key"
    , subParser "convert-bcc-address-key" $
        Opt.info pKeyConvertBccAddressSigningKey $
          Opt.progDesc $ "Convert a bcc-address extended signing key "
  ...
  where
    ...
    pColeKeyType =
          Opt.flag' (ColePaymentKey NonLegacyColeKeyFormat)
            (  Opt.long "cole-payment-key-type"
            <> Opt.help "Use a Cole-era payment key."
            )
      <|> Opt.flag' (ColePaymentKey LegacyColeKeyFormat)
            (  Opt.long "legacy-cole-payment-key-type"
            <> Opt.help "Use a Cole-era payment key, in legacy SL format."
            )
      <|> Opt.flag' (ColeGenesisKey NonLegacyColeKeyFormat)
            (  Opt.long "cole-genesis-key-type"
            <> Opt.help "Use a Cole-era genesis key."
            )
      <|> Opt.flag' (ColeGenesisKey LegacyColeKeyFormat)
      ...
      <|> Opt.flag' (ColeDelegateKey LegacyColeKeyFormat)
    ...
    pColeKeyFile =
      ...
      <|> (AVerificationKeyFile <$> pColeVerificationKeyFile)

    pColeSigningKeyFile :: Parser SigningKeyFile
    pColeSigningKeyFile =
      SigningKeyFile <$>
        Opt.strOption
          (  Opt.long "cole-signing-key-file"
          <> Opt.metavar "FILE"
          <> Opt.help "Input filepath of the Cole-format signing key."
          <> Opt.completer (Opt.bashCompleter "file")
          )
    ...
    pColeVerificationKeyFile =
      VerificationKeyFile <$>
        Opt.strOption
          (  Opt.long "cole-verification-key-file"
          <> Opt.metavar "FILE"
          <> Opt.help "Input filepath of the Cole-format verification key."
          <> Opt.completer (Opt.bashCompleter "file")
          )
    ...
    pBccAddressKeyType =
          Opt.flag' BccAddressSophiePaymentKey
            (  Opt.long "sophie-payment-key"
            <> Opt.help "Use a Sophie-era extended payment key."
            )
      <|> Opt.flag' BccAddressSophieStakeKey
            (  Opt.long "sophie-stake-key"
            <> Opt.help "Use a Sophie-era extended stake key."
            )
      <|> Opt.flag' BccAddressIcarusPaymentKey
            (  Opt.long "icarus-payment-key"
            <> Opt.help "Use a Cole-era extended payment key formatted in the Icarus style."
            )
      <|> Opt.flag' BccAddressColePaymentKey
            (  Opt.long "cole-payment-key"
            <> Opt.help "Use a Cole-era extended payment key formatted in the deprecated Cole style."
            )
...
pGovernanceCmd =
 asum
  ...
  where
    ...
    pMIRPot =
          Opt.flag' Sophie.ReservesMIR
            (  Opt.long "reserves"
            <> Opt.help "Use the reserves pot."
            )
      <|> Opt.flag' Sophie.TreasuryMIR
            (  Opt.long "treasury"
...
pTransferAmt =
    Opt.option (readerFromParsecParser parseEntropic)
      (  Opt.long "transfer"
      <> Opt.metavar "ENTROPIC"
  ...
  asum
    [ subParser "key-gen-genesis"
        (Opt.info pGenesisKeyGen $
           Opt.progDesc "Create a Sophie genesis key pair")
    ...
    , subParser "create"
        (Opt.info pGenesisCreate $
           Opt.progDesc ("Create a Sophie genesis file from a genesis "
                      ++ "template and genesis/delegation/spending keys."))
    , subParser "create-staked"
        (Opt.info pGenesisCreateStaked $
           Opt.progDesc ("Create a staked Sophie genesis file from a genesis "
                      ++ "template and genesis/delegation/spending keys."))
    , subParser "hash"
    ...
    pInitialSupplyNonDelegated =
      ...
      Entropic <$>
        Opt.option Opt.auto
          (  Opt.long "supply"
          <> Opt.metavar "ENTROPIC"
          <> Opt.help "The initial coin supply in Entropic which will be evenly distributed across initial, non-delegating stake holders."
          )

    pInitialSupplyDelegated :: Parser Entropic
    pInitialSupplyDelegated =
      fmap (Entropic . fromMaybe 0) $ Opt.optional $
        Opt.option Opt.auto
          (  Opt.long "supply-delegated"
          <> Opt.metavar "ENTROPIC"
          <> Opt.help "The initial coin supply in Entropic which will be evenly distributed across initial, delegating stake holders."
          <> Opt.value 0
          )
...
pAddressKeyType =
    Opt.flag' AddressKeySophie
      (  Opt.long "normal-key"
      <> Opt.help "Use a normal Sophie-era key (default)."
      )
  <|>
    Opt.flag' AddressKeySophieExtended
      (  Opt.long "extended-key"
      <> Opt.help "Use an extended ed25519 Sophie-era key."
      )
  <|>
    Opt.flag' AddressKeyCole
      (  Opt.long "cole-key"
      <> Opt.help "Use a Cole-era key."
      )
  <|>
    pure AddressKeySophie
...
pWithdrawal balance =
    (\(stakeAddr,entropic) maybeScriptFp -> (stakeAddr, entropic, maybeScriptFp))
 ...
 where
   helpText = "The reward withdrawal as StakeAddress+Entropic where \
   ...
   parseWithdrawal =
     (,) <$> parseStakeAddress <* Parsec.char '+' <*> parseEntropic
...
pBccEra = asum
  [ Opt.flag' (AnyBccEra ColeEra)
      (  Opt.long "cole-era"
      <> Opt.help "Specify the Cole era"
      )
  , Opt.flag' (AnyBccEra SophieEra)
      (  Opt.long "sophie-era"
      <> Opt.help "Specify the Sophie era"
      )
  , Opt.flag' (AnyBccEra EvieEra)
      (  Opt.long "evie-era"
      <> Opt.help "Specify the Evie era"
      )
  , Opt.flag' (AnyBccEra JenEra)
      (  Opt.long "jen-era"
      <> Opt.help "Specify the Jen era (default)"
      )
  , Opt.flag' (AnyBccEra AurumEra)
      (  Opt.long "aurum-era"
      <> Opt.help "Specify the Aurum era"
      )

    -- Default for now:
  , pure (AnyBccEra JenEra)
  ]
...
pTxFee =
  Entropic . (fromIntegral :: Natural -> Integer) <$>
    Opt.option Opt.auto
      (  Opt.long "fee"
      <> Opt.metavar "ENTROPIC"
      <> Opt.help "The fee amount in Entropic."
      )
...
pTxSophieWitnessCount =
  TxSophieWitnessCount <$>
    Opt.option Opt.auto
      ...
      <> Opt.metavar "NATURAL"
      <> Opt.help "The number of Sophie key witnesses."
      )
...
pTxColeWitnessCount =
  TxColeWitnessCount <$>
    Opt.option Opt.auto
      (  Opt.long "cole-witness-count"
      <> Opt.metavar "NATURAL"
      <> Opt.help "The number of Cole key witnesses (default is 0)."
      <> Opt.value 0
      )
  ...
  where
    ...
    pByAddress =
        Opt.option (readerFromParsecParser parseAddressAny)
          (  Opt.long "address"
          <> Opt.metavar "ADDRESS"
          <> Opt.help "Filter by Bcc address(es) (Bech32-encoded)."
          )
...
pFilterByStakeAddress =
    Opt.option (readerFromParsecParser parseStakeAddress)
      (  Opt.long "address"
      <> Opt.metavar "ADDRESS"
      <> Opt.help "Filter by Bcc stake address (Bech32-encoded)."
      )
...
pColeAddress =
    Opt.option
      (Opt.eitherReader deserialise)
        (  Opt.long "address"
        <> Opt.metavar "STRING"
        <> Opt.help "Cole address (Base58-encoded)."
        )
  where
    deserialise :: String -> Either String (Address ColeAddr)
    deserialise =
      maybe (Left "Invalid Cole address.") Right
        . deserialiseAddress AsColeAddress
        . Text.pack
...
eDNSName str =
  ...
  case Sophie.textToDns (toS str) of
    Nothing -> Left "DNS name is more than 64 bytes"
    Just dnsName -> Right . Text.encodeUtf8 . Sophie.dnsToText $ dnsName
...
pProtocolParametersUpdate =
  ProtocolParametersUpdate
    ...
    <*> pure mempty -- TODO aurum: separate support for cost model files
...
pMinUTxOValue =
    Opt.option (readerFromParsecParser parseEntropic)
      ...
      <> Opt.metavar "NATURAL"
      <> Opt.help "The minimum allowed UTxO value (Sophie to Jen eras)."
      )
...
pExtraEntropy =
      Opt.option (Just <$> readerFromParsecParser parseOptimumNonce)
        (  Opt.long "extra-entropy"
        <> Opt.metavar "HEX"
        <> Opt.help "Optimum extra entropy, as a hex byte string."
        )
  <|> Opt.flag' Nothing
        (  Opt.long "reset-extra-entropy"
        <> Opt.help "Reset the Optimum extra entropy to none."
        )
  where
    parseOptimumNonce :: Parsec.Parser OptimumNonce
    parseOptimumNonce = makeOptimumNonce <$> parseEntropyBytes
...
pUTxOCostPerWord =
    Opt.option (readerFromParsecParser parseEntropic)
      ...
      <> Opt.metavar "ENTROPIC"
      <> Opt.help "Cost in entropic per unit of UTxO storage (from Aurum era)."
      )
...
pMaxValueSize =
  Opt.option Opt.auto
    ...
    <> Opt.metavar "INT"
    <> Opt.help "Max size of a multi-asset value in a tx output (from Aurum \
                \era)."
    )
