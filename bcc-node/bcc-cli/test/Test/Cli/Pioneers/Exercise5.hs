...
module Test.Cli.Pioneers.Exercise5
  ( tests
  ) where
...
prop_createLegacyZeroTxOutTransaction :: Property
prop_createLegacyZeroTxOutTransaction = propertyOnce . H.moduleWorkspace "tmp" $ \tempDir -> do
  -- Key filepaths
...
tests =
  H.checkSequential
    $ H.Group "Pioneers Example 5"
        [ ("prop_createLegacyZeroTxOutTransaction", prop_createLegacyZeroTxOutTransaction)
        ]
