...
module Test.Golden.Sophie.Transaction.CalculateMinFee
...
golden_sophieTransactionCalculateMinFee :: Property
golden_sophieTransactionCalculateMinFee = propertyOnce $ H.moduleWorkspace "tmp" $ \tempDir -> do
  protocolParamsJsonFile <- noteInputFile "test/data/golden/sophie/transaction-calculate-min-fee/protocol-params.json"
  txBodyFile <- noteInputFile "test/data/golden/sophie/tx/txbody"
  minFeeTxtFile <- noteTempFile tempDir "min-fee.txt"

  minFeeTxt <- execBccCLI
    [ "transaction","calculate-min-fee"
    , "--tx-in-count", "32"
    , "--tx-out-count", "27"
    , "--cole-witness-count", "5"
    , "--witness-count", "10"
