...
module Test.Golden.Sophie.Genesis.Create
  ( golden_sophieGenesisCreate
  ) where
...
parseMaxEntropicSupply :: J.Value -> J.Parser Int
parseMaxEntropicSupply = J.withObject "Object" $ \o -> o J..: "maxEntropicSupply"
...
golden_sophieGenesisCreate :: Property
golden_sophieGenesisCreate = propertyOnce $ do
  H.moduleWorkspace "tmp" $ \tempDir -> do
    sourceGenesisSpecFile <- noteInputFile "test/data/golden/sophie/genesis/genesis.spec.json"
    ...
    actualSupply <- H.evalEither $ J.parseEither parseMaxEntropicSupply actualJson
