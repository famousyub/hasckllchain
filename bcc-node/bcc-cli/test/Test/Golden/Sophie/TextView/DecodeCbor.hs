...
module Test.Golden.Sophie.TextView.DecodeCbor
  ( golden_sophieTextViewDecodeCbor
  ) where
...
golden_sophieTextViewDecodeCbor :: Property
golden_sophieTextViewDecodeCbor = propertyOnce $ H.moduleWorkspace "tmp" $ \tempDir -> do
  unsignedTxFile <- noteInputFile "test/data/golden/sophie/tx/unsigned.tx"
  decodedTxtFile <- noteTempFile tempDir "decoded.txt"

  -- Defaults to signing a Mainnet transaction.

  decodedTxt <- execBccCLI
