...
import           Bcc.Prelude hiding (lines, readFile, stderr, stdout)
...
-- | Execute bcc-cli via the command line.
--
...
execBccCLI
  ...
  -> m String
  -- ^ Captured stdout
execBccCLI = GHC.withFrozenCallStack $ H.execFlex "bcc-cli" "BCC_CLI"
...
noteInputFile filePath = withFrozenCallStack $ do
  H.annotate $ bccCliPath <> "/" <> filePath
  return filePath
