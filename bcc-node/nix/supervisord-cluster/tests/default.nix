...
, mkCluster
  ...
  inherit (pkgs) mkCluster bcc-cli bcclib-py bcc-node;
  ...
  pythonDeps = with pkgs.python3Packages; [ pkgs.python3 requests pyyaml docopt bcclib-py ];
  clusterDeps = [ cluster'.start cluster'.stop pkgs.jq bcc-cli bcc-node ];
  ...
  mkClusterTest = name: testScript: pkgs.runCommand name { buildInputs = clusterDeps ++ pythonDeps; } ''
    export BCC_NODE_SOCKET_PATH=${stateDir}/bft0.socket
    start-cluster
    export NETWORK_MAGIC=$(jq .networkMagic < ${stateDir}/sophie/genesis.json)
    cp ${testScript} script.py
    chmod -R u+w ${stateDir}
    cp ${../genesis-utxo.vkey} ${stateDir}/sophie/genesis-utxo.vkey
    cp ${../genesis-utxo.skey} ${stateDir}/sophie/genesis-utxo.skey
    cp ${./base.py} base.py
