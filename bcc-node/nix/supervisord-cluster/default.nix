let
  ...
  cacheDirDefault    = "${__getEnv "HOME"}/.cache/bcc-workbench";
    ...
    { ## Generic Nix bits:
      ...
      finaliseNodeService =
        { name, i, isProducer, ... }: svc: recursiveUpdate svc
          ({
            ...
            executable     = "cabal run exe:bcc-node --";
      ...
      finaliseNodeConfig =
        { port, ... }: cfg: recursiveUpdate cfg
          ({
            AurumGenesisFile    = "../genesis/aurum-genesis.json";
            SophieGenesisFile   = "../genesis.json";
            ColeGenesisFile     = "../genesis/cole/genesis.json";
