...
EOF
    ...
    start-cluster )
        ...
        if test ! -v BCC_NODE_SOCKET_PATH
        then export  BCC_NODE_SOCKET_PATH=$(backend_supervisor get-node-socket-path "$dir")
        fi

        echo -n "workbench:  supervisor:  waiting for $BCC_NODE_SOCKET_PATH to appear: " >&2
        while test ! -S $BCC_NODE_SOCKET_PATH
        do echo -n '.'; sleep 1
        done >&2
        echo >&2
