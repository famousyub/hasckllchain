...
case "${op}" in
    ...
    actually-genesis )
        ...
        bcc-cli genesis create --genesis-dir "$dir"/ \
        ...
        bcc-cli genesis create-staked "${params[@]}"

        ## TODO: temporary step for Aurum
        jq '
          { bccPerUTxOWord: 0
          , executionPrices:
            { prMem:              1
          ...
          }' --null-input > "$dir"/aurum-genesis.json
    ...
    finalise-cache-entry )
        ...
        genesis_cole "$system_start_epoch" "$dir"
...
genesis_cole()
{
    ...
    ' --null-input > "$dir"/cole-protocol-params.json
    cli_args=(
        ## Note that these parameters are irrelevant by now.
        --genesis-output-dir         "$dir"/cole
        --protocol-parameters-file   "$dir"/cole-protocol-params.json
    ...
    rm -rf "$dir"/cole
    bcc-cli cole genesis genesis "${cli_args[@]}"
}
