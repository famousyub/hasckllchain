...
rewriteGenesisSpec testnetOptions startTime =
  rewriteObject
    ...
    . HM.insert "maxEntropicSupply" (J.toJSON @Integer (maxEntropicSupply testnetOptions))
...
testnet testnetOptions H.Conf {..} = do
  ...
  let optimumNodesN = show @Int <$> [1 .. numOptimumNodes testnetOptions]
  let optimumNodes = ("node-optimum" <>) <$> optimumNodesN
  let poolNodesN = show @Int <$> [1 .. numPoolNodes testnetOptions]
  let poolNodes = ("node-pool" <>) <$> poolNodesN
  let allNodes = optimumNodes <> poolNodes :: [String]
  let numOptimumNodes = L.length allNodes :: Int
  let userPoolN = poolNodesN -- User N will delegate to pool N

  allPorts <- H.noteShowIO $ IO.allocateRandomPorts numOptimumNodes
  ...
  H.copyFile
    (base </> "configuration/chairman/sophie-only/configuration.yaml")
  ...
  void $ H.execCli
    ...
    , "--testnet-magic", show @Int testnetMagic
    , "--genesis-dir", tempAbsPath
    , "--gen-genesis-keys", show numOptimumNodes
  ...
  let numNodesStr = map show [1..numOptimumNodes]
  ...
  forM_ optimumNodesN $ \n -> do
    H.createFileLink (tempAbsPath </> "delegate-keys/delegate" <> n <> ".skey") (tempAbsPath </> "node-optimum" <> n </> "operator.skey")
    H.createFileLink (tempAbsPath </> "delegate-keys/delegate" <> n <> ".vkey") (tempAbsPath </> "node-optimum" <> n </> "operator.vkey")
    H.createFileLink (tempAbsPath </> "delegate-keys/delegate" <> n <> ".counter") (tempAbsPath </> "node-optimum" <> n </> "operator.counter")
    H.createFileLink (tempAbsPath </> "delegate-keys/delegate" <> n <> ".vrf.vkey") (tempAbsPath </> "node-optimum" <> n </> "vrf.vkey")
    H.createFileLink (tempAbsPath </> "delegate-keys/delegate" <> n <> ".vrf.skey") (tempAbsPath </> "node-optimum" <> n </> "vrf.skey")
  ...
  forM_ userPoolN $ \n -> do
    ...
    void $ H.execCli
      ...
      , "--tx-out", userNAddr <> "+" <> show @Integer (maxEntropicSupply testnetOptions)
  ...
  forM_ allNodes $ \node -> do
    ...
    void $ H.createProcess =<<
      ( H.procNode
        ...
        , "--sophie-kes-key", tempAbsPath </> node </> "kes.skey"
        , "--sophie-vrf-key", tempAbsPath </> node </> "vrf.skey"
        , "--sophie-operational-certificate" , tempAbsPath </> node </> "node.cert"
