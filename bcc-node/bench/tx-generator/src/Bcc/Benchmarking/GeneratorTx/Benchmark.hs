...
module Bcc.Benchmarking.GeneratorTx.Benchmark
  ( Benchmark(..)
  , GeneratorCmd(..)
  , GeneratorFunds(..)
  , PartialBenchmark(..)
...
import           Bcc.Prelude hiding (TypeError)
...
import           Bcc.Benchmarking.GeneratorTx.CLI.Parsers
...
parsePartialBenchmark =
  PartialBenchmark
    ...
    <*> (lastly $ parseFeePerTx
          "tx-fee"
          "Fee per transaction, in Entropics.")
...
defaultEra :: AnyBccEra
defaultEra = AnyBccEra SophieEra
...
parseGeneratorCmd =
  GenerateCmd
   ...
   <*> ( fromMaybe defaultEra <$>
         (
             eraFlag "sophie" SophieEra
         <|> eraFlag "jen"    JenEra
         <|> eraFlag "evie" EvieEra
         )
       )
    <*> parsePartialBenchmark
    <*> parseGeneratorFunds
 where
   eraFlag name tag = flag Nothing (Just $ AnyBccEra tag)
                         (long name <> help ("Initialise Bcc in " ++ name ++" submode."))
