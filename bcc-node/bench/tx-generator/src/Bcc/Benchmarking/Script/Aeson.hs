...
module Bcc.Benchmarking.Script.Aeson
where
...
import           Bcc.Api (AnyBccEra(..), BccEra(..))
...
import           Bcc.Benchmarking.Types (NumberOfTxs(..), TPSRate(..))
...
instance FromJSON AnyBccEra where
  parseJSON = withText "AnyBccEra" $ \case
    "Cole"   -> return $ AnyBccEra ColeEra
    "Sophie" -> return $ AnyBccEra SophieEra
    "Evie" -> return $ AnyBccEra EvieEra
    "Jen"    -> return $ AnyBccEra JenEra
    "Aurum"    -> return $ AnyBccEra AurumEra
    era -> parseFail ("Error: Cannot parse JSON value '" <> Text.unpack era <> "' to AnyBccEra.")
